...
t    = ascii.read(inputcat,fill_values=[('NA', '0', 'Sloan_r'),('NA', '0', 'Sloan_i'),('NA', '0', 'i_err'),('NA', '0', 'r_err'),('NA', '0', 'Johnson_V'),('NA', '0', 'Johnson_B'),('NA', '0', 'Sloan_g'),('NA', '0', 'gerr'),('NA', '0', 'B_err'),('NA', '0', 'Verr')])
...
for m in range(len(sdssr)) :
	...
	nn = sdssr[m] - 1.2444*(sdssr[m] - sdssi[m]) - 0.3820 
	rconvert.append(mm)
	iconvert.append(nn)
	vab.append(apassv[m] - 0.044)
	bab.append(apassb[m] - 0.163)
	rerr.append(np.sqrt(float(sdssrerr[m])**2. + float(sdssierr[m])**2.) + 0.0072**2.)   #include conversion error
	ierr.append(np.sqrt(float(sdssrerr[m])**2. + float(sdssierr[m])**2.) + 0.0078**2.)
