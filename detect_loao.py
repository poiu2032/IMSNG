...
#	sexcom='sex -c phot.sex '+inimage+' -CATALOG_NAME '+inimage[:-5]+'.cat -PARAMETERS_NAME  phot.param -psf_name '+tmpimage[:-5]+'.psf -seeing_fwhm '+str(psfexxml(tmpimage[:-5]+'.xml'))+' -DETECT_THRESH '+detecthred+' -ANALYSIS_THRESH '+analthred+' -PHOT_APERTURES '+apersize 
        sexcom='sex -c /home/sylee/sextractor/phot.sex '+inimage+' -CATALOG_NAME '+inimage[:-5]+'.cat -PARAMETERS_NAME  /home/sylee/sextractor/phot.param  -DETECT_THRESH '+detecthred+' -ANALYSIS_THRESH '+analthred+' -PHOT_APERTURES '+apersize
	os.system(sexcom)
...
#	subcat=ascii.read(inimage[:-5]+'.cat')
	...
	print filename[:-5] ,'1231231231231231231'
	#ra=subcat['ALPHA_J2000']
	#dec=subcat['DELTA_J2000']
	#starname=subcat['NUMBER']
...
#	pl.imshow(data, cmap = mpl.cm.gray,origin='lower')
	...
	fig.add_label(0.5,0.95,image[20:],relative=True,color='orange',weight='bold')
...
#	pl.savefig('jpg/'+image[:-5]+'.jpg')
	...
	pl.savefig('jpg/'+image[:-5]+'.jpg', dpi=200)
...
# reference
fig = aplpy.FITSFigure('ref.fits')
objname = fits.getheader('ref.fits')['OBJECT']
fig.set_theme('pretty')               #'publication'or 'pretty'
...
os.system('cp jpg/hd*com.jpg jpg/com/')
os.system('cp jpg/ref.jpg jpg/com/')
curdirec = os.getcwd()
galname=curdirec.split('/')[-3]
os.system('mkdir /d1/itelescope_data/t52/jpg/'+galname)
os.system('cp -r jpg/*.jpg /d1/itelescope_data/t52/jpg/'+galname)
os.system('cp -r jpg/com /d1/itelescope_data/t52/jpg/'+galname)
