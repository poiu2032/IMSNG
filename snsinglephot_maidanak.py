...
outfile = dirhere[3]+'-'+dirhere[5]+'-photb.txt'    ##  'NGC0337-R-phota.txt'
...
def sedual(measure) : 
	...
	detecthred   = '1.5'        ## sextractor parameter first for LSGT
	analthred    = '2'
	seeing       = '1.5'
	apersize     = '11.2,18.7,9,26.2'  # 3", 5", 7" aperture 0.2679"/pix
...
def zpcal(im) :
	...
	index=np.where((dist > inradius) & (dist < outradius) & (atme < 0.2)  & (flag < 5.0) )  ## inside ouradius and outside inradius, error < 0.2,   flag value < 5
	index=list(index[0])
	print len(index)
