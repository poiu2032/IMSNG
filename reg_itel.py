...
subprocess.call('ls Calibrate*.fits',shell=True)
subprocess.call('ls Calibrate*.fits > obj.list.done',shell=True)
subprocess.call('gethead Calibrate*.fits DATE-OBS > obj.list.date',shell=True)
...
for g in range(len(gg)) :	
	inlist=list(subdic.get(g)[0])
	
	for mm in range(len(subdic.get(g)[0])) :
		print 'input image : ', inlist[mm]		
		output='r'+inlist[mm]
		ref=inlist[0]
		print 'reference = ', ref 
		wrcom="wregister "+inlist[mm]+" "+ref+" "+output+'\n'
		infile=inlist[mm]		
		print wrcom
		f.write(wrcom)		
