...
os.system('ls cfdobj.*.fits > obj.list')
...
for n in range(len(addlist)):
	...
	decdd=decd.degree	
	com='solve-field \''+addlist[n]+'\' --temp-dir /home/sylee/Desktop/LOAO/red/astrometry/dump --scale-unit arcsecperpix --scale-low 0.3 --scale-high 0.9 --ra '+str(radd)+' --dec '+str(decdd)+', --radius 0.5 --no-plots --new-fits '+'a'+addlist[n]+' --overwrite --use-sextractor\n'
	print com
	f.write(com)
...
subprocess.call('ls acfdobj.*.fits > obj.done',shell=True)

orinum=subprocess.check_output('ls cfdobj.*.fits | wc -l',shell=True)

resnum=subprocess.check_output('ls acfdobj.*.fits | wc -l',shell=True)

print "from ",orinum[:-1],"files , ",resnum[:-1] ,"files are solved"
print "all done"

os.system('rm /home/sylee/Desktop/LOAO/red/astrometry/dump/*')
